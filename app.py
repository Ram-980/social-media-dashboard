...
server = app.server
# Optional: Enable dev tools for better error reporting on Render
...
app.layout = html.Div([
    ...
    dbc.Row([
        dbc.Col([
            html.H5("Filter by:"),
            html.Label("Gender"),
            dcc.Dropdown(id='gender-filter', options=[{'label': g, 'value': g} for g in genders], value=genders, multi=True),
            html.Label("Age Group"),
            dcc.Dropdown(id='age-filter', options=[{'label': a, 'value': a} for a in age_groups], value=age_groups, multi=True),
            html.Label("Addiction Level"),
            dcc.Dropdown(id='addiction-filter', options=[{'label': l, 'value': l} for l in addiction_levels], value=addiction_levels, multi=True),
            html.Label("Academic Level"),
            dcc.Dropdown(id='academic-filter', options=[{'label': al, 'value': al} for al in academic_levels], value=academic_levels, multi=True),
...
def update_graphs(selected_genders, selected_ages, selected_addiction, selected_academic):
    ...
    def empty_fig(title):
        return go.Figure().update_layout(
            title=title,
            annotations=[dict(text="No data available", x=0.5, y=0.5, showarrow=False, font_size=16)]
        )

    # Ensure none of the inputs are empty
    if not (selected_genders and selected_ages and selected_addiction and selected_academic):
        return tuple([empty_fig(title) for title in [
            "Most Used Social Media Platforms", "Addiction Score Distribution by Gender",
            "Addiction Score vs Academic Performance", "Sleep Hours by Addiction Level and Gender",
            "Mental Health Score by Addiction Level and Gender", "Cluster Grouping of Students"
        ]])
    ...
    if 'Affects_Academic_Performance' in filtered.columns and 'Addicted_Score' in filtered.columns:
        academic_data = filtered.copy()
        academic_data['Affects_Academic_Performance_Numeric'] = academic_data['Affects_Academic_Performance'].map({'Yes': 1, 'No': 0})
        academic_data = academic_data.dropna(subset=['Affects_Academic_Performance_Numeric'])
